...
# Allow the creation of const LifePatterns without a setindex!
...
A type representing a rectangular group of cells to be inserted into a `LifeGrid`.

See [`insert!`](@ref), which is specialized to put `LifePatterns` into `LifeGrid`s
efficiently.
...
Return an `m×n` `LifePattern` with all cells inactive.

---

    LifePattern(pattern::AbstractMatrix)

Return a `LifePattern` with a grid of bools or numbers.

As with `LifeGrid`s, non-zero or true cells in `pattern` correspond to living cells.
"""
...
`jrange` should be of less length than `C` has bits.
"""
Base.@propagate_inbounds @inline function bitchunk(
    ...
    data1 = data[I1] << shift
    data2 = data[I2] >> (nbits(P) - shift)
    x = ifelse(I1 == I2, data1, data1 | data2)
