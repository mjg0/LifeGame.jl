...
function benchmarkstep(sidelengths, parallel)
    ...
    t = time()
    while time()-t < 60
        step!(LifeGrid(10_000, 10_000); parallel=parallel)
    end
    ...
    return [begin
        ...
        result = @benchmark step!($lg; parallel=$parallel)
...
sidelengths = (small = 1:500,
               large = 5 .* 2 .^ (1:16))
results = (smallserial   = benchmarkstep(sidelengths.small, false),
...
smallplotkw = (title             = "Cell updates per nanosecond",
               legend_position   = :topleft,
               xlabel            = "LifeGrid side length",
               ylabel            = "Updates/ns",
               margin            = (5, :mm),
               size              = (600, 400))
largeplotkw = (xscale            = :log10,
